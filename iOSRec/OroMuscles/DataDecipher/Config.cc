...
void Config::Read(const string & fileName)
{
  ...
  while (parser.ParseLine()) {
    ...
    for (int i=1; i<parser.GetItemCount(); i++) {
      if (parser.AsString(i)[0] == '#')
	break;
      full += parser.AsString(i);
      full += " ";
    } 
...
void Config::GetFullValue(string & out, const string & key)
{
  int index = Index(key);
  if (index < 0) {
    out = "";
  ...
  out = m_conf[index].ValueStringFull();
}
