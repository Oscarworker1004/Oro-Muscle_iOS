...
bool CMAsciiReadFileStream::Open(const CMString & name)
{
  ...
  if (strstr(name, ".gz") != NULL) {
    ...
    cmd += name;
#ifdef _WIN64
    ThrowException("CMAsciiReadFileStream::Open: gzip compression not supported!");
#else
	//m_pFile = popen(cmd, "r");
	//throw; // For cygwin!!!!!!!!!!!!!!!
    ThrowException("CMAsciiReadFileStream::Open: gzip compression not supported!");
...
bool CMReadFileStream::ReadString(CMString & string)
{
  ...
  bool bSuccess = true;
  if (fread((void *)pData, len-1, 1, m_pFile) != 1) {
    bSuccess = false;
    string = "";
  } else {
    pData[len-1] = 0;
...
bool CMTokenizer::Tokenize(CMPtrStringList & result, const CMString & source, bool bNoRemoveMult)
{
  const MCL_TCHAR * pszBuffer = source;
  ...
  for (int i=0; i<=len; i++) {
    ...
    if (IsDelimiter(&pszBuffer[i], inc, idx) || i == len) {
      ...
      k = 0;
      if (__mccl_strcmp(szTemp, _TMCL("")) != 0 || bNoRemoveMult) {
	CMString * pNewString = new CMString;
	*pNewString = szTemp;
	result.add(pNewString);
        __mccl_strcpy(szTemp, _TMCL(""));
...
#define FSCHUNK 4194304

CMWriteBufferStream::CMWriteBufferStream()
{
  m_bytesProcessed = 0;
}
...
void CMWriteBufferStream::Align(int bytes)
{
  int n = m_bytesProcessed % bytes;
  char one = 0;
  for (int i=0; i<n; i++)
    Write(one);
}

bool CMWriteBufferStream::Open(const CMString & name)
{
  m_buffer.reshape(FSCHUNK);
  m_bytesProcessed = 0;
  return true;

}

const char * CMWriteBufferStream::Get() const
{
  return m_buffer.data();
}

bool CMWriteBufferStream::Close()
{
  m_bytesProcessed = 0;
  return true;
}

bool CMWriteBufferStream::IsOpen()
{
  return m_buffer.length() > 0;
}

bool CMWriteBufferStream::IsEnd()
{
  return false;
}

bool CMWriteBufferStream::WriteSimpleType(const void * pData, long lenInBytes)
{
  int i;
  while (m_bytesProcessed + lenInBytes >= m_buffer.length())
    m_buffer.reshape(m_buffer.length()+FSCHUNK);
  for (i=0; i<lenInBytes; i++) {
    m_buffer[m_bytesProcessed] = ((const char*)pData)[i];
    m_bytesProcessed++;
  }
  return true;
...
bool CMWriteBufferStream::WriteBlob(const void * pData, long lenInElements, long elSize)
{
  return WriteSimpleType(pData, lenInElements*elSize);
}

bool CMWriteBufferStream::WriteString(const CMString & string)
{
  int l = string.length();
  Write(l);
  return WriteSimpleType((const void*)string.Data(), l);
}


//=================================================

CMReadBufferStream::CMReadBufferStream()
{
  m_bytesProcessed = 0;
  m_reserve = 0;
}

void CMReadBufferStream::Align(int bytes)
{
  int n = m_bytesProcessed % bytes;
  char one;
  for (int i=0; i<n; i++)
...
void CMReadBufferStream::SetReserve(int reserve)
{
  m_reserve = reserve;
}

bool CMReadBufferStream::Open(const CMString & name)
{
  m_bytesProcessed = 0;
  m_buffer.reshape(0);
  return true;
...
bool CMReadBufferStream::IsOpen()
{
  return m_bytesProcessed < m_buffer.length();
}

bool CMReadBufferStream::IsEnd()
{
  //cout << "DEBUG " << m_bytesProcessed + m_reserve << " >= " << m_buffer.length() << endl;
  return (m_bytesProcessed + m_reserve) >= m_buffer.length();
}

bool CMReadBufferStream::Set(const char * pBuffer, int sizeInBytes)
{
  m_bytesProcessed = 0;
  m_buffer.reshape(sizeInBytes);
  for (int i=0; i<sizeInBytes; i++)
    m_buffer[i] = pBuffer[i];
  return true;
}

void CMReadBufferStream::Compact()
{
  int n = m_buffer.length() - m_bytesProcessed;

  for (int i=m_bytesProcessed; i<m_buffer.length(); i++) {
    m_buffer[i - m_bytesProcessed] = m_buffer[i];
  }

  m_buffer.reshape(n, true);
  
  m_bytesProcessed = 0;
}

bool CMReadBufferStream::Append(const char * pBuffer, int sizeInBytes)
{
  int n = m_buffer.length();
  m_buffer.reshape(n + sizeInBytes, true);

  //cout << "DEBUG Resize: " << m_buffer.length() << endl;
  for (int i=0; i<sizeInBytes; i++)
    m_buffer[i+n] = pBuffer[i];
  return true;

}

bool CMReadBufferStream::ReadSimpleType(void * pData, long lenInBytes)
{
  int i;
  if (m_bytesProcessed + lenInBytes > m_buffer.length())
    return false;
  
  //cout << "DEBUG " << m_bytesProcessed << " " << m_buffer.length() << endl;
  
  for (i=0; i<lenInBytes; i++) {
    ((char*)pData)[i] = m_buffer[m_bytesProcessed];
    m_bytesProcessed++;
  }
  return !IsEnd();
}

bool CMReadBufferStream::ReadBlob(void * pData, long lenInElements, long elSize)
{
  return ReadSimpleType(pData, lenInElements*elSize);
}

bool CMReadBufferStream::ReadString(CMString & string)
{
  string = "";
  ...
  Read(l);
  for (int i=0; i<l; i++) {
    string += m_buffer[m_bytesProcessed];
    m_bytesProcessed++;
  }
